...
<head>
    ...
    <title>Shantha Motors Enterprise Portal - 3-Way Matching</title>
    <style>
        :root {
            --bg-main: #0F2A4A;
            --sidebar-bg: #0d2440;
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .sidebar { width: 260px; background-color: var(--sidebar-bg); padding: 25px 15px; display: flex; flex-direction: column; justify-content: space-between; border-right: 2px solid var(--border-color); z-index: 10; }
        .brand-block { text-align: center; margin-bottom: 25px; }
        .brand-logo-svg { width: 65px; height: 65px; margin-bottom: 8px; fill: #ffffff; }
        .brand-name { color: #ffffff; font-size: 20px; font-weight: 800; letter-spacing: 1.5px; text-transform: uppercase; }
        .brand-sub { color: #d8c5ff; font-size: 11px; letter-spacing: 4px; text-transform: uppercase; margin-top: 2px; font-weight: 600; }

        .nav-list { display: flex; flex-direction: column; gap: 12px; width: 100%; }
        .nav-link { text-decoration: none; width: 100%; display: block; }
        .nav-btn { width: 100%; padding: 12px 14px; border-radius: 25px; font-size: 11px; font-weight: 700; text-align: center; cursor: pointer; border: none; display: block; }
        ...
        .topbar { background-color: var(--topbar-bg); height: 65px; display: flex; align-items: center; justify-content: center; color: white; font-weight: 700; letter-spacing: 1.5px; font-size: 18px; text-transform: uppercase; }
        .content-workspace { flex: 1; padding: 20px 25px; overflow-y: auto; background-color: var(--bg-main); display: flex; flex-direction: column; gap: 20px; }
        ...
        .panel-header-strip { background-color: var(--table-header); color: white; padding: 10px 15px; border-radius: 4px; font-size: 12px; font-weight: 700; text-transform: uppercase; margin-bottom: 15px; display: flex; justify-content: space-between; align-items: center; }

        .custom-table-container { overflow-x: auto; border-radius: 6px; border: 1px solid var(--border-color); }
        .custom-table { width: 100%; border-collapse: collapse; font-size: 13px; }
        .custom-table th { background-color: var(--table-header); color: white; text-align: left; padding: 12px 15px; font-weight: 600; text-transform: uppercase; font-size: 11px; }
        .custom-table td { padding: 12px 15px; border-bottom: 1px solid var(--border-color); color: var(--text-color); }
...
<body data-theme="dark">
    ...
    <div class="sidebar">
        <div>
            ...
            <div class="nav-list">
                <a href="/mari" class="nav-link"><button class="nav-btn inactive">Purchase Requisition and Approval</button></a>
                ...
                <a href="/malacaste" class="nav-link"><button class="nav-btn active">Goods Receipt and Invoice Matching</button></a>
            </div>
        </div>
    </div>
    ...
    <div class="main-viewport">
        ...
        <div class="content-workspace">
            ...
            <!-- 3. INTERACTIVE 3-WAY MATCHING INSPECTION -->
            <div class="data-panel">
                <div class="panel-header-strip">
    ...
    <script>
        ...
        function renderMainTable() {
            ...
            records.forEach(r => {
                ...
                if (r.approval_status === 'Approved') {
                    ...
                    actionHtml = `<button class="btn-locked" title="Cannot approve due to mismatch">🔒 Locked</button>`;
                }
        ...
        function approvePO(id) {
            ...
            if (confirm(`Approve payment for ${r.po_number}?`)) {
